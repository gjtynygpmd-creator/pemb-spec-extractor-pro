...
FIELD_RULES: tuple[FieldRule, ...] = (
    ...
    FieldRule("Codes & Loads", "Basic Wind Speed", (
        ...
        # "mph" was optional and the gap allowed newlines, so a label like
        # "BASIC WIND SPEED" on a drawing would bind to the next bare number on the
        # sheet (e.g. an eave height of 24'-0") and report "24 mph". Requiring the
        # unit and forbidding an intervening newline eliminates that false bind.
        r"(?:ultimate\s+)?(?:basic\s+)?wind\s+speed[^\S\n]*[:=\-–—]?[^\S\n]*(\d{2,3})[^\S\n]*mph",
        r"V\s*ult[^\S\n]*[:=\-]?[^\S\n]*(\d{2,3})[^\S\n]*mph",
    ), 0.94, ("structural_notes", "specification"), ("13",)),
...
GENERIC_LABEL_RULES: tuple[FieldRule, ...] = (
    FieldRule("Geometry", "Building Width", (r"\b(?:bldg\.?|building)\s+(?:overall\s+)?width[^\S\n]*[:=\-]?[^\S\n]*([^\n|]{2,40})",), 0.78),
    FieldRule("Geometry", "Building Length", (r"\b(?:bldg\.?|building)\s+(?:overall\s+)?length[^\S\n]*[:=\-]?[^\S\n]*([^\n|]{2,40})",), 0.78),
    ...
    FieldRule("Codes & Loads", "Roof Snow Load", (r"\b(?:flat\s+roof\s+snow\s+load|roof\s+snow\s+load|pf)[^\S\n]*[:=\-]?[^\S\n]*([^\n|]{1,30})",), 0.87),
    FieldRule("Codes & Loads", "Roof Live Load", (r"\broof\s+live\s+load[^\S\n]*[:=\-]?[^\S\n]*([^\n|]{1,30})",), 0.87),
    FieldRule("Codes & Loads", "Collateral Load", (r"\bcollateral(?:\s+dead)?\s+load[^\S\n]*[:=\-]?[^\S\n]*([^\n|]{1,30})",), 0.86),
    FieldRule("Codes & Loads", "Seismic Design Category", (r"\bseismic\s+design\s+category[^\S\n]*[:=\-]?[^\S\n]*([^\n|]{1,20})",), 0.89),
    FieldRule("Codes & Loads", "Site Class", (r"\bsite\s+class[^\S\n]*[:=\-]?[^\S\n]*([^\n|]{1,20})",), 0.87),
)
...
# Diagnostic labels used to judge whether a page's embedded text is worth parsing
# with the regex engine. A born-digital spec page hits many of these; a drawing whose
# text is scattered callouts and a title block hits few, so it is routed to vision.
_RICH_TEXT_LABELS = (
    r"building\s+code", r"risk\s+category", r"wind\s+speed", r"snow\s+load",
    r"live\s+load", r"collateral\s+load", r"seismic\s+design\s+category",
    r"site\s+class", r"exposure", r"occupancy", r"insulation",
    r"roof\s+panel", r"wall\s+panel", r"eave\s+height", r"roof\s+slope",
    r"building\s+width", r"building\s+length", r"specification", r"section\s+\d",
)


def count_rich_text_labels(text: str) -> int:
    """Number of distinct diagnostic PEMB labels present in the text."""
    if not text:
        return 0
    return sum(1 for pattern in _RICH_TEXT_LABELS if re.search(pattern, text, re.I))


def page_has_rich_text_layer(text: str, min_chars: int = 200, min_labels: int = 3) -> bool:
    """True when the embedded text is long AND label-dense enough for the regex path.

    Pages that fail this test (image-only, scanned, or drawings whose text is a
    handful of scattered callouts) should be routed to the vision path instead of
    being parsed by regex or silently dropped.
    """
    text = (text or "").strip()
