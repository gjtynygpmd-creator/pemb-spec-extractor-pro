...
def build_vision_instructions() -> str:
    ...
    lines: list[str] = []
    lines.append(
        "You are an expert construction estimator for pre-engineered metal buildings "
        "(PEMB), structural steel, and metal panel systems. You are reading one sheet from "
        "a project document, which may be an invitation to bid (ITB), a project information "
        "report (e.g. ConstructConnect Insight), an internal PEMB quote form, structural "
        "drawings, or a written specification, with two-column layouts, schedules, and "
        "title blocks. Read all columns, tables, schedules, and dimension callouts.\n\n"
        "Return ONLY a JSON object with a single key \"fields\" whose value is a list of "
        "objects, each with:\n"
        "  - field_id: the exact id from the list below\n"
        "  - value: the value exactly as written, including units and dropdown selections\n"
        "  - confidence: a number from 0 to 1\n"
        "  - source_note: a short phrase describing where on the sheet it was found\n\n"
        "Rules:\n"
        "  - Use ONLY field_id values from the list. Omit any field not present. Never invent a value.\n"
        "  - For enum fields, choose one of the allowed values.\n"
        "  - Ignore boilerplate (standard AIA contract clauses, insurance requirement lists, "
        "generic bidding instructions) unless it contains a requested value.\n"
        "  - Ignore placeholder values like \"-None-\" and treat them as absent.\n"
        "  - Do not confuse a dimension (e.g. an eave height like 24'-0\") with a load or a "
        "wind speed, and do not read a table column header (e.g. \"RISK CATEGORY IV\") as an "
        "assigned value.\n\n"
