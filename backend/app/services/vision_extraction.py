...
def _run_with_timeout(fn, timeout_s: int):
    """Run fn() with a hard wall-clock timeout. Raises TimeoutError if it does not
    finish in time. The worker thread cannot be force-killed, but the caller stops
    waiting so the job continues (belt-and-suspenders alongside the SDK client timeout).
    """
...
def extract_from_png(png_bytes: bytes) -> VisionResult:
    ...
    try:
        ...
        call = (lambda: _call_anthropic(b64)) if _provider() == "anthropic" else (lambda: _call_openai(b64))
        raw = _run_with_timeout(call, settings.page_timeout_seconds)
        ...
        log.warning("Vision call timed out after %ss (%s)", settings.page_timeout_seconds, _provider())
        return VisionResult(reason=f"vision timeout after {settings.page_timeout_seconds}s")
...
def extract_from_page(page, dpi: int | None = None) -> VisionResult:
    ...
    if not vision_available():
        return VisionResult(reason=f"vision unavailable ({describe_config()}; feature off, no key, or SDK missing)")
    try:
        png = _run_with_timeout(lambda: render_page_png(page, dpi=dpi), settings.page_timeout_seconds)
    except concurrent.futures.TimeoutError:
        log.warning("Page render timed out after %ss", settings.page_timeout_seconds)
        return VisionResult(reason=f"render timeout after {settings.page_timeout_seconds}s")
